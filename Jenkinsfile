pipeline {
  agent {
    label {
      label "master"
      customWorkspace "/u01/data/jenkins/workspace/sonar-poc/${BRANCH_NAME}"
    }
  }
            ...
            withSonarQubeEnv('Wynyard Sonar') {
				sh '''
                	/u01/data/jenkins/tools/hudson.plugins.sonar.SonarRunnerInstallation/Wynyard_Sonar/bin/sonar-scanner \
              			-Dsonar.projectKey=Sonar_POC -Dsonar.projectName=sonar_poc \
                        ...
                        -Dsonar.modules=sonar_poc -Dsonar_poc.sonar.projectName=sonar_poc -Dsonar_poc.sonar.projectBaseDir=./src -Dsonar_poc.sonar.sources=./main/java -Dsonar_poc.sonar.java.binaries=../target/classes
                '''
            }
          }
